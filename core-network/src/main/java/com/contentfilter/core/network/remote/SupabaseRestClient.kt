...
class SupabaseRestClient
    ...
    constructor(
        ...
        suspend fun selectAll(table: SupabaseTable): RemoteResult<JSONArray> = withContext(Dispatchers.IO) {
            executeArray(
        ...
        suspend fun patchById(
            ...
            val request = requestBuilder("/rest/v1/${table.tableName}?id=eq.$encodedId")
                ?: return@withContext RemoteResult.Failure(OfflineUserMessage, retryable = true)
            val body = json.toString().toRequestBody(JsonMediaType)
            try {
                httpClient.newCall(
