...
begin
    ...
    if matched_code.id is null then
        raise exception 'Token de administrador inválido, vencido o ya usado';
    end if;
    ...
    if existing_user.id is not null then
        if auth.uid() is distinct from existing_user.id then
            raise exception 'Ese email ya existe. Ingresá la contraseña correcta o usá otro email';
        end if;
