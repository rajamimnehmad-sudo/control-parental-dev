...
printf '\n=== Glosh Remote PIN-only 19 gate ===\n'
printf 'Repo: %s\nHEAD: %s\n' "$REPO_ROOT" "$(git -C "$REPO_ROOT" rev-parse HEAD)"

printf '\n[1/5] Product architecture + pairing/session stability + stable DEV signing guard\n'
...
if grep -Eq 'GuideOverlayController|GuideNotification|SamsungGuideStep|SettingsNavigator|requestOverlayPermission' "$MAIN"; then echo "ERROR: MainActivity volvió a depender de la guía/Bubble superseded." >&2; exit 3; fi
if ! grep -q 'showPairingInput' "$MAIN" || ! grep -q 'ACTION_SUBMIT_CODE' "$MAIN" || ! grep -q 'renderReconnecting' "$MAIN"; then echo "ERROR: falta contrato PIN-only/reconnecting." >&2; exit 3; fi
...
printf 'PASS: PIN-only + fresh pairing + persistent identity + ADB/relay reconnect + screen lease (versionCode %s)\n' "$VERSION_CODE"
...
{
  echo "TASK=REMOTE-PIN-ONLY-19"; echo "RESULT=PASS_AUTOMATED"; echo "HEAD=$HEAD_SHA"; echo "ARCHITECTURE_GUARD=PASS"; echo "PYTHON_TESTS=PASS"; echo "ANDROID_UNIT_TESTS=PASS"; echo "LINT=PASS"; echo "ASSEMBLE=PASS"; echo "PAIRING_STABILITY=PASS"; echo "PERSISTENT_ADB_IDENTITY=PASS"; echo "ADB_RECONNECT=PASS"; echo "RELAY_RECONNECT=PASS"; echo "SCREEN_AWAKE_LEASE=PASS"; echo "VERSION_CODE=$VERSION_CODE"; echo "DEV_SIGNER_SHA256=$APK_CERT_SHA256"; echo "APK=$FINAL_APK"; echo "APK_SIZE_BYTES=$APK_SIZE"; echo "APK_SHA256=$APK_SHA";
