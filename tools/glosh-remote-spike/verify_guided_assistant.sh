...
FINAL_APK="$APK_DIR/GloshRemote-MAINTENANCE-20-DEV35.apk"
REPORT="$APK_DIR/REMOTE-DEVICE-OWNER-PREFLIGHT-20B-report.txt"
PYTHON_BIN="${PYTHON_BIN:-python3}"
VENV_DIR="$(mktemp -d "${TMPDIR:-/tmp}/glosh-pin-only-python.XXXXXX")"
MIN_DEV_VERSION_CODE=35
...
{
  echo "TASK=REMOTE-DEVICE-OWNER-PREFLIGHT-20B"; echo "RESULT=PASS_AUTOMATED"; echo "HEAD=$HEAD_SHA"; echo "ARCHITECTURE_GUARD=PASS"; echo "PYTHON_TESTS=PASS"; echo "ANDROID_UNIT_TESTS=PASS"; echo "LINT=PASS"; echo "ASSEMBLE=PASS"; echo "NOTIFICATION_PIN_INPUT=PASS"; echo "WIRELESS_SETTINGS_HANDOFF=PASS"; echo "PAIRING_STABILITY=PASS"; echo "SIMPLE_CORE_PAIR_CONNECTTLS_WHOAMI_RELAY=PASS"; echo "ENCRYPTED_MAINTENANCE_SHELL=PASS"; echo "ARTIFACT_BOUND_PROVISIONING=PASS"; echo "ANDROID_ICU_REGEX_GUARD=PASS"; echo "SESSION_SCREEN_CPU_WIFI_LEASE=PASS"; echo "PERSISTENT_ADB_IDENTITY=PASS"; echo "VERSION_CODE=$VERSION_CODE"; echo "DEV_SIGNER_SHA256=$APK_CERT_SHA256"; echo "APK=$FINAL_APK"; echo "APK_SIZE_BYTES=$APK_SIZE"; echo "APK_SHA256=$APK_SHA";
