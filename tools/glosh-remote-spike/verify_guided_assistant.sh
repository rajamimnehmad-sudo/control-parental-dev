...
printf '\n=== Glosh Remote PIN-only 19 local-connect gate ===\n'
printf 'Repo: %s\nHEAD: %s\n' "$REPO_ROOT" "$(git -C "$REPO_ROOT" rev-parse HEAD)"

printf '\n[1/5] Product architecture + notification PIN + local TLS connect + pairing/session stability + stable DEV signing guard\n'
...
if ! grep -q 'SERVICE_TYPE_TLS_CONNECT' "$ADB_MANAGER" || ! grep -q 'discoverTlsEndpoint' "$ADB_MANAGER" || ! grep -q 'AdbConnectEndpointPolicy.isUsable' "$ADB_MANAGER" || ! grep -q 'connect(endpoint.host(), endpoint.port())' "$ADB_MANAGER"; then echo "ERROR: conexión ADB post-pairing no espera un endpoint TLS connect válido." >&2; exit 3; fi
if grep -q 'connectTls(context' "$ADB_MANAGER"; then echo "ERROR: volvió el connectTls one-shot vulnerable a service-lost port=-1." >&2; exit 3; fi
...
printf 'PASS: PIN-only + notification reply + valid-only TLS connect discovery + persistent identity + reconnect + screen lease (versionCode %s)\n' "$VERSION_CODE"
...
{
  echo "TASK=REMOTE-PIN-ONLY-19F-LOCAL-CONNECT"; echo "RESULT=PASS_AUTOMATED"; echo "HEAD=$HEAD_SHA"; echo "ARCHITECTURE_GUARD=PASS"; echo "PYTHON_TESTS=PASS"; echo "ANDROID_UNIT_TESTS=PASS"; echo "LINT=PASS"; echo "ASSEMBLE=PASS"; echo "NOTIFICATION_PIN_INPUT=PASS"; echo "WIRELESS_SETTINGS_HANDOFF=PASS"; echo "PAIRING_STABILITY=PASS"; echo "ADB_TLS_CONNECT_DISCOVERY=PASS"; echo "LOCAL_ADB_BEFORE_RELAY=PASS"; echo "PERSISTENT_ADB_IDENTITY=PASS"; echo "ADB_RECONNECT=PASS"; echo "RELAY_RECONNECT=PASS"; echo "SCREEN_AWAKE_LEASE=PASS"; echo "VERSION_CODE=$VERSION_CODE"; echo "DEV_SIGNER_SHA256=$APK_CERT_SHA256"; echo "APK=$FINAL_APK"; echo "APK_SIZE_BYTES=$APK_SIZE"; echo "APK_SHA256=$APK_SHA";
