...
class MediaBarrierContractTest {
    ...
    fun `extension starts before content and updates in place`() {
        ...
        assertContains(manifest, "\"version\": \"2.0.61\"")
    ...
    fun `native fullscreen authority preserves cover and closes the exact video grant`() {
        ...
        assertContains(activity, "handleVideoLabFullscreenReady(")
        assertContains(videoProtocol, "video-lab-fullscreen-state")
        assertContains(videoProtocol, "video-lab-fullscreen-ready")
        assertContains(css, "video[data-glosh-dag-video-fullscreen=\"true\"]")
        assertContains(css, "position: fixed !important")
        assertContains(css, "object-fit: contain !important")
