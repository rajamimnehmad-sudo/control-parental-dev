...
class MediaBarrierContractTest {
    ...
    fun `extension runs before page content and in every frame`() {
        ...
        assertContains(manifest, "\"version\": \"1.28.0\"")
    ...
    fun `image responses remain withheld during bounded native inspection`() {
        ...
        assertContains(background, "MAX_ACTIVE_IMAGE_STREAMS = 64")
        assertContains(background, "MAX_INTERCEPT_CAPTURE_BUDGET_BYTES = 8 * 1024 * 1024")
        assertContains(background, "reserveInterceptCaptureBytes")
        assertContains(background, "releaseInterceptCaptureBytes")
        ...
        assertContains(background, "DOCUMENT_CURRENT_MESSAGE = \"media-document-current\"")
        assertContains(background, "DOCUMENT_RETIRED_MESSAGE = \"media-document-retired\"")
        assertContains(background, "tabId: documentState.tabId")
        assertContains(background, "documentToken: documentState.documentToken")
        assertContains(background, "settleStream")
        assertFalse(background.contains("SAFE_IMAGE_PLACEHOLDER"))
    ...
    fun `capacity pressure defers safely into a bounded visible first queue`() {
        val background = extensionRoot.resolve("background.js").readText()
        val script = extensionRoot.resolve("barrier.js").readText()
        val style = extensionRoot.resolve("barrier.css").readText()
        ...
        assertContains(background, "MAX_PRIORITY_HINTS_PER_DOCUMENT = 512")
        assertContains(background, "MAX_CONSECUTIVE_VISIBLE_INTERCEPTS = 4")
        assertFalse(background.contains("MAX_FALLBACK_DECISIONS"))
        ...
        assertContains(background, "priority = \"background\"")
        assertContains(background, "priority,")
        assertContains(background, "fetchFallbackDecision(task.sourceUrl, task.priority, task.documentState)")
        assertContains(background, "CLIENT_METRIC_MESSAGE")
        assertContains(background, "nativeRoundTripMillis")
        assertContains(background, "transportPath: \"fallback\"")
        assertContains(background, "transportPath: \"intercept\"")
        assertContains(background, "sentAtEpochMillis: Date.now()")
        ...
        assertContains(background, "drainAnalysisQueues")
        assertContains(background, "takeNextInterceptTask")
        assertContains(background, "promoteInterceptTasks")
        assertContains(background, "PRIORITY_HINT_MESSAGE = \"media-priority-hint\"")
        assertContains(background, "documentState.priorityHints")
        assertContains(background, "isCurrentDocumentState(documentState)")
        assertContains(background, "promoteFallbackTask")
        assertFalse(background.contains("filter.disconnect()"))
        ...
        assertContains(script, "prepareImageForAsyncDecoding")
        assertContains(script, "sendMediaPriorityHint")
        assertContains(script, "sendPriorityHintsForElement")
        assertContains(script, "PRIORITY_HINT_MESSAGE = \"media-priority-hint\"")
        assertFalse(script.contains("fetchpriority\", \"high"))
        assertFalse(script.contains("loading\", \"eager"))
        ...
        assertContains(script, "decisionsBySource.get(sourceUrl) === \"block\"")
        assertContains(script, "ERROR_ACCESSIBLE_DESCRIPTION")
        assertContains(style, "data-glosh-dag-media-host=\"filtered\"")
        assertContains(style, "prefers-reduced-motion: reduce")
        assertFalse(style.contains("Imagen no disponible"))
        assertFalse(style.contains("blur(28px)"))
    ...
    fun `media analysis protocol is native and fail closed`() {
        ...
        assertContains(css, "animation: none !important")
    ...
    fun `performance evidence is isolated to the initial window of each document`() {
        ...
        assertContains(background, "currentDocumentForSender(sender, message.documentToken)")
    ...
    @Test
    fun `media states keep waiting filtered and technical surfaces distinct without leaking pixels`() {
        val css = extensionRoot.resolve("barrier.css").readText()
        val script = extensionRoot.resolve("barrier.js").readText()
        val background = extensionRoot.resolve("background.js").readText()
        val activity =
        ...
        assertFalse(script.contains("APPROVED_PRESENTATION_SELECTOR"))
        assertContains(script, "clearWaitingMediaHostsAround")
        assertContains(script, "reconcileMediaHostState(current)")
        assertContains(script, "attributeName === MEDIA_HOST_ATTRIBUTE")
        assertContains(script, "previousHost === host")
        ...
        assertFalse(css.contains("blur(28px)"))
        assertFalse(css.contains("Imagen no disponible"))
        assertFalse(css.contains("clip-path: polygon"))
        assertFalse(script.contains("presentationState === \"filtered\""))
        assertContains(script, "decisionsBySource.get(sourceUrl) === \"block\"")
        assertContains(
            css,
            "html:not([data-glosh-dag-background-probe]) [data-glosh-dag-media-host]::after",
        )
