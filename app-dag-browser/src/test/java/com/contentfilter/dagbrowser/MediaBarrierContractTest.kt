...
class MediaBarrierContractTest {
    ...
    fun `native fullscreen authority preserves cover and closes the exact video grant`() {
        ...
        assertFalse(activity.contains("enterProtectedFallbackFullscreen"))
        assertFalse(activity.contains("protectedFallbackFullscreen"))
        assertFalse(activity.contains("SCREEN_ORIENTATION_SENSOR_LANDSCAPE"))
        assertFalse(activity.contains("exitProtectedFallbackFullscreen"))
        assertFalse(videoProtocol.contains("video-lab-fullscreen-state"))
        assertFalse(css.contains("data-glosh-dag-video-fullscreen"))
        assertFalse(css.contains("position: fixed !important"))
    ...
    fun `normal DEV does not pay diagnostic tracing cost`() {
        ...
        assertContains(activity, "armVideoLabCoverWhenSurfaceReady")
        assertContains(activity, "attempt < VideoLabSurfaceReadyRetries")
        assertContains(activity, "beginVideoLabClose(\"invalid_surface_rect\")")
    ...
    fun `covered video reuses one prepared raster authority without revealing frames`() {
        ...
        assertContains(activity, "DagVideoDocumentRearmPolicy.supports(reason)")
        assertContains(activity, "DagVideoDocumentRearmPolicy.allow(")
        assertContains(activity, "postVideoLabConfig(sourcePort, enabled = true)")
        assertContains(activity, "DagVideoProtectionActivationPolicy.senderEnabled")
        assertContains(activity, "videoDocumentPorts.connect(")
        assertContains(activity, "videoDocumentPorts.disconnect(senderTab.id, sourcePort)")
        assertContains(activity, "rearmVideoProtectionFor(tab)")
        assertContains(activity, "postVideoLabConfig(document.port, enabled = enabled, fixture = document.fixture)")
        ...
        assertContains(activity, "videoBlockedPlaceholder.showNativePlayback(key)")
        ...
        assertContains(blockedPlaceholder, "fun showNativePlayback(")
        assertFalse(blockedPlaceholder.contains("showFullscreenButton"))
        assertFalse(browserLayout.contains("video_lab_fullscreen_button"))
        ...
        assertContains(videoLifecycle, "!nativeAuthorityStarted && state.enabled")
        assertContains(videoLifecycle, "Native owns the close/revoke boundary")
        assertContains(videoLifecycle, "state.isolationLocked = true")
        assertContains(videoLifecycle, "dependencies.quarantineBlockedAuthority(record)")
        assertContains(videoLab, "blockQuarantine.allows(video)")
        assertContains(videoCapture, "dependencies.onFrameBlocked(record)")
        ...
        assertContains(rasterPolicy, "GloshiaPreparedRasterPolicy.decide")
        assertContains(rasterPolicy, "shared.basis.toDagBasis()")
