...
internal enum class ChromeH20RendererAmplificationMode {
    AttrStress,
    ChildStress,
    MarkupStress,
    SvgStress,
...
internal class ChromeH20RendererAmplificationFixture {
    ...
    fun responseFor(request: ChromePhotosProxyRequest): ChromePhotosFixtureResponse? {
        ...
        return when (path) {
            ...
            RawPagePath -> {
                documents.incrementAndGet()
                html("h20-renderer-amplification-off", document(mode(request.target), requireSelfShieldTrace = false))
            }
            ScriptPath -> {
                scripts.incrementAndGet()
                response(
                    "h20-renderer-amplification-script",
                    JavaScript,
                    script(
                        mode(request.target),
                        requireSelfShieldTrace = !request.target.contains("raw=1"),
                    ).toByteArray(),
                )
            }
    ...
    internal fun document(
        mode: ChromeH20RendererAmplificationMode,
        requireSelfShieldTrace: Boolean = true,
    ): String =
        "<!doctype html><html><head><meta charset=\"utf-8\"><title>H20 RENDERER ${mode.name}</title>" +
            "<script defer src=\"$ScriptPath?mode=${mode.wireName()}${if (requireSelfShieldTrace) "" else "&raw=1"}\"></script></head><body>" +
    ...
    internal fun script(
        ...
        (if (requireSelfShieldTrace) ChromeMediaShieldBootstrap.selfShieldOriginalScriptStartedScript() else "") +
            """
            ;(()=>{'use strict';const MODE='${mode.wireName()}',ROOT=document.getElementById('fixture-root'),STARTED=performance.now();let siteMutations=0;
            ...
            const markup=()=>{for(let i=0;i<240;i+=1){const node=document.createElement('article');node.innerHTML='<span>markup-'+i+'</span>';ROOT.appendChild(node);node.remove();siteMutations+=3}};
            ...
            if(MODE==='ATTR_STRESS')attr();else if(MODE==='CHILD_STRESS')child();else if(MODE==='MARKUP_STRESS')markup();else if(MODE==='SVG_STRESS')svg();else if(MODE==='SHADOW_STRESS')shadow();else{attr();child();markup();svg();shadow()}
            queueMicrotask(async()=>{const durationMicros=Math.round((performance.now()-STARTED)*1000);document.dispatchEvent(new Event('${ChromeMediaShieldRendererMetricsScript.SnapshotEvent}'));try{await fetch('$ReportPath',{method:'POST',headers:{'Content-Type':'text/plain'},body:'MODE='+MODE+',SITE_MUTATIONS='+siteMutations+',DURATION_MICROS='+durationMicros})}catch(_){}})})();
            """.trimIndent().replace("\n", "")
    ...
    companion object {
        ...
        const val RawPagePath = "/webh20/renderer-amplification-off"
        ...
        private val ReportPattern =
            Regex(
                "MODE=(ATTR|CHILD|MARKUP|SVG|SHADOW|MIXED)_STRESS," +
                    "SITE_MUTATIONS=[0-9]{1,7},DURATION_MICROS=[0-9]{1,12}",
            )
    }
}
