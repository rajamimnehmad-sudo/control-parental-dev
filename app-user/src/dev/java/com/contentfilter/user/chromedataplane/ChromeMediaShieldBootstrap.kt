...
internal object ChromeMediaShieldBootstrap {
    ...
    private val ScriptTemplate =
        ...
        const iconSprite=(svg)=>{if(!nativeHas.call(svg,'width')||!nativeHas.call(svg,'height'))return false;if(NativeNumber(nativeGet.call(svg,'width'))!==0||NativeNumber(nativeGet.call(svg,'height'))!==0)return false;const defs=elementQuery.call(svg,'defs');if(!defs)return false;const nodes=copyList(elementQueryAll.call(svg,'*'),nodeListLength);let identified=false;for(let index=0;index<nodes.length;index+=1){const node=nodes[index];if(node===defs)continue;if(elementClosest.call(node,'defs')!==defs)return false;if(nativeGet.call(node,'id'))identified=true}return identified};
        const safeIcon=(svg)=>{rendererMetric(26);try{const descendants=copyList(elementQueryAll.call(svg,'*'),nodeListLength);if(descendants.length>511)return false;const nodes=[svg],ids=new Set(),ambiguousIds=new Set(),refs=[];for(let index=0;index<descendants.length;index+=1)append(nodes,descendants[index]);
        ...
        if(name==='id'){if(!internalReference('#'+value))return false;if(invoke(SetHas,ids,[value]))invoke(SetAdd,ambiguousIds,[value]);invoke(SetAdd,ids,[value])}if(name==='href'||name==='xlink:href'){if(!internalReference(value))return false;append(refs,slice(value,1))}
        ...
        if(name==='xmlns:xlink'&&value!=='http://www.w3.org/1999/xlink')return false;if(name==='xml:space'&&value!=='default'&&value!=='preserve')return false;if((name==='data-skapa'||name==='data-name')&&(!value||value.length>128))return false;
        ...
        installed=sealCompatibleMethod(Node.prototype,'removeChild',function(node){if(parentOf(node)!==this)return node;if(node===shieldStyle||node===curtainStyle)return node;if(containsProtected(node))deny();return nodeRemove.call(this,node)})&&installed;
        ...
        installed=seal(owner,'remove',function(){if(this===shieldStyle||this===curtainStyle)return;if(containsProtected(this))deny();return invoke(remove,this,[])})&&installed}
