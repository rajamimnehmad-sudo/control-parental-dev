...
class ChromeMediaShieldBootstrapContractTest {
    ...
    fun `bounded framework metadata does not hide original inline SVG icons`() {
        ...
        assertContains(script, "const iconSprite=(svg)=>")
        assertContains(script, "!iconDimensions(svg)&&!elementQuery.call(svg,'symbol')&&!iconSprite(svg)")
        assertContains(script, "(targetTag!=='symbol')&&(targetTag!=='g')")
        assertContains(script, "!elementClosest.call(target,'defs')")
