...
class ChromeOriginalUiSvgStylePolicyTest {
    ...
    fun `inert original icon styles preserve exact bytes`() {
        ...
        val result = assertIs<ChromeOriginalUiSvgValidation.Valid>(ChromeOriginalUiSvgValidator().validate(svg, "image/svg+xml"))
    ...
    fun `resource syntax escapes variables active selectors and fragmented styles fail closed`() {
        ...
        for (style in styles) {
            ...
            assertIs<ChromeOriginalUiSvgValidation.Invalid>(ChromeOriginalUiSvgValidator().validate(svg, "image/svg+xml"), style)
        }
    }
}
