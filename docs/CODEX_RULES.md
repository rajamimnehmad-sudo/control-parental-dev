...
- Todo candidato o version de DAG que cambie navegador, carga, WebView o imagenes debe ejecutar antes de entregarse la matriz fija Fravega, Mimo y Cheeky en el mismo telefono objetivo. Cada URL usa `codexperf`, limpia el cache HTTP de WebView y fuerza `LOAD_NO_CACHE`; una medicion asistida por cache no es valida. Registrar por separado `page_analysis_ready` (pagina/texto), `viewport_images_ready` (fotos iniciales decididas) y `page_visible` (momento en que el usuario ve pagina y fotos), junto con dispositivo, Android, fecha, variante y cualquier timeout/rotura en `docs/compatibility/results/dag-performance-history.md`; una pagina que no completa cuenta como regresion, no como dato ausente.
