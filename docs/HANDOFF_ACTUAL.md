...
- App Usuario agrega medicion tzniut local y calibrable de mangas por encima del codo y prendas por encima de la rodilla. El detector embebido de pose se ejecuta solo con contexto femenino suficiente y combina landmarks con piel local; los cruces fuertes se difuminan y los cercanos se envian a revision. En SM-A235M, Zara abrio y navego; la primera inferencia de pose midio 558 ms y las siguientes aproximadamente 70 ms. No se activo Accessibility durante la prueba.
...
- Backend aplicado solamente a `syeycayasyufedwoprea`: migracion `20260719113929_dag_calibration_closed_loop`, RLS activa sin lectura para anon/authenticated, asesores de seguridad y rendimiento sin errores, y Edge Function `dag-calibration` v11 activa. Una invocacion no autenticada/invalida fallo cerrada con HTTP 400. Pendiente antes de publicar: commit/push, workflows, Superweb oficial, manifiestos, hashes y reinstalacion del APK publico exacto.
