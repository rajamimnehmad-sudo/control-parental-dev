...
## DAG-V2-BROWSER-FOUNDATION-02 - corrección local validada; Android CI y prueba física pendientes - 2026-07-25
...
- El fallo remoto original de Android CI (`30180080622`, job `89735049544`) era un test DEV ubicado por error en `src/test`, ejecutado por el `test` global también en Beta; se movió a `src/testDev`. El comando remoto completo y las tareas específicas de App Usuario y `:feature-dag2` pasan localmente; WebView debugging está ausente, los manifiestos fusionados confirman Lab sólo en DEV y el archivo DAG v1 verifica sus 28 archivos.
- ADB detectó únicamente un SM-A235M, que no se instaló ni modificó; el SM-S908E requerido no estaba conectado. Por eso no se atribuyen métricas a Frávega, Mimo o Cheeky y siguen pendientes las diez aperturas con caché normal, diez sin caché y veinte segundos por sitio en el S22 Ultra.
