...
- Supabase DEV `syeycayasyufedwoprea` contiene la migración `20260726041736`, Edge Function `dag-v2-calibration`, bucket privado y tablas `dag_v2_calibration_*`, con RLS, Service Role sólo en Edge, autorización del dispositivo, rate limit, deduplicación y auditoría. No existe lectura pública ni escritura a DAG v1.
- Gradle global y específico, 9 tests Deno, formato/check Deno, verificadores de contrato/empaquetado, archivo v1 y asesores de seguridad/rendimiento de los objetos nuevos resultaron correctos.
