...
- El Service Worker global sólo atribuye una solicitud mediante evidencia de generación, `Origin`, `Referer`, URL y origen registrados. Sin sesión devuelve `null`; una funcional no atribuible devuelve `null` sin métricas y una visual no atribuible o cancelada devuelve placeholder neutro sin atribuirla a la sesión vigente.
- El análisis completo se inicia una sola vez y mantiene `fullPageAnalysisCount == 1`. La Navigation API, `hashchange` y `popstate` cubren rutas SPA; cuando Navigation API no existe se usa una única comprobación periódica acotada de `location.href`, sin monkey-patchear History ni volver a recorrer el DOM.
- La búsqueda y el análisis textual reutilizan `DagContentClassifier` mediante un contrato neutral DEV: consulta, URL, dominio, resultado y página conservan las prohibiciones canónicas; una consulta adulta se bloquea antes de Brave y una regla Admin común de permitir no reemplaza una prohibición adulta.
- HTML, CSS, JavaScript, JSON, XHR, `fetch`, fuentes, RSC, manifests, módulos y scripts de Service Worker evitan el pipeline visual. El guard compartido bloquea de inmediato esquemas no HTTPS y hosts IP literales o especiales no públicos, y la navegación principal resuelve DNS fuera del callback síncrono. Permanece documentado un riesgo residual: WebView resuelve por su cuenta subrecursos funcionales con hostname, por lo que no se declara mitigación total de DNS rebinding sin una capa futura de conexión fijada.
- En este lote no existe ruta visual aprobada: raster y SVG reciben un placeholder constante generado localmente, sin descargar, conservar, difuminar ni derivar píxeles originales. Se retiró la regla global que anulaba todos los `background-image`; la neutralización es dirigida. Al retirar, bloquear o cerrar el contenido, un host de Activity libera exactamente una vez el WebView aunque coincidan `onRelease` y `onDestroy`. Un gate de generación evita que una Activity anterior destruida tarde cierre la reapertura actual.
- El fallo remoto original de Android CI (`30180080622`, job `89735049544`) era un test DEV ubicado por error en `src/test`, ejecutado por el `test` global también en Beta; se movió a `src/testDev`. El run correctivo `30182570727`, job `89741575612`, completó build DEV y `Calidad compartida` correctamente en 9m59s. El comando remoto completo y las tareas específicas de App Usuario y `:feature-dag2` también pasan localmente; WebView debugging está ausente, los manifiestos fusionados confirman Lab sólo en DEV y el archivo DAG v1 verifica sus 28 archivos.
- La validación física autorizada se realizó exclusivamente en el SM-A235M `R58T34V31AE`, Android 14/API 34, build `UP1A.231005.007.A235MUBSAEYB1`. La APK DEV local se instaló in-place, conservó `versionCode 279` y tiene SHA-256 `ea6403d607b259ddde431fbad399315e6f94297b73e2156c450a24cc8d2e614a`.
- Se completaron 60/60 aperturas: diez normales y diez con `Sin caché DEV` para Frávega, Mimo y Cheeky, todas durante al menos 20 segundos. Promedio hasta estructura visible: Frávega 3,886 s normal y 3,163 s sin caché; Mimo 0,417 s y 0,605 s; Cheeky 1,989 s y 2,119 s. En todos los documentos `full_page_analysis_count == 1`, las nuevas imágenes recibieron placeholder individual y no apareció ningún píxel fotográfico real. No hubo crash, ANR ni `renderer_gone`; WebView debugging siguió desactivado.
- Menús, categorías, tabs, modal, carrusel, scroll/lazy loading, enlaces, back, forward y recarga funcionaron. Mimo aplicó filtro y acordeones como SPA sin repetir análisis. Frávega mantuvo `#__next`, no mostró `Cannot read properties of undefined (reading 'length')` y sus scripts/JSON/RSC continuaron; el rótulo de filtro de la categoría probada no abrió un panel. En Cheeky, placeholders móviles muy altos dificultaron acceder a algunos controles inferiores sin colapsar la página. La temperatura de batería subió de 29,0 °C a 30,0 °C durante la secuencia conectada por USB.
- La prueba descubrió y corrigió dos causas generales: una navegación principal con `Accept` mixto podía ser clasificada como raster, y la destrucción/reapertura rápida podía conservar estado o cerrar la generación nueva. Se agregaron regresiones para prioridad de documento principal y lifecycle. No se agregó ninguna excepción por dominio.
- Es un candidato DEV no publicado y todavía no está aprobado externamente. No se incrementó `versionCode`, no se modificó App Admin, Supabase ni Production, no se publicó APK ni se borraron datos, y no se inició Calibración DEV v2, dataset, entrenamiento o Lote 3. Mientras el PR siga abierto, el rollback es dejar de usar la rama; una reversión posterior requeriría otro ticket aprobado.
...
- Corrección de confiabilidad: alcanzar límites devuelve `Full`/`TooLarge`/`PersistenceFailure` sin borrar pendientes ni consumir el candidato; los permanentes dejan un recibo cifrado separado y no bloquean B/C, mientras los temporales conservan el orden. La expiración local es explícita y nunca se usa para aceptar inmediatamente una etiqueta nueva.
- Supabase DEV `syeycayasyufedwoprea` contiene la migración base `20260726041736`, la correctiva forward-only `20260726111615`, Edge Function `dag-v2-calibration` v2, bucket privado y tablas `dag_v2_calibration_*`. Una exacta `pending` se reanuda con o sin objeto, una `rejected` se restaura para upload y `ready` es idempotente sin auditoría duplicada.
- Gradle global y específico, 15 tests Deno, prueba SQL transaccional, formato/check Deno, verificadores de contrato/empaquetado, archivo v1 y asesores de seguridad/rendimiento resultaron correctos.
- Validación física exclusiva en SM-A235M `R58T34V31AE`, Android 14/API 34, APK in-place `versionCode 279`, SHA-256 `50a7b2ea23ae99d291fce86e38b3076f780aac913d91b93407b6443659f82ece`: Frávega, Mimo y Cheeky mantuvieron estructura, placeholders, controles y `full_page_analysis_count == 1`; activar/desactivar, scroll, recarga explícita, cola, preview y cierre no expusieron una imagen dentro de WebView. Sin crash, ANR ni `renderer_gone`.
...
- DEV 276 publicado y validado en SM-A235M: los modelos visuales se preparan mientras WebView descarga HTML y las fotos permitidas ya no generan miniaturas JPEG de calibracion sin uso. La referencia Samsung paso de aproximadamente 9,5 s en DEV 275 a 5,9 s en el primer recorrido y 4,0 s en la repeticion, con PSS aproximada 555-561 MB, heap nativo 342-348 MB y 26,2 C. No cambian decisiones, umbrales ni la espera cerrada de cada imagen. El modelo TFLite compacto fue 3,5 veces mas rapido y coincidio en 19/19 fotos seguras, pero no reemplaza al profesional por faltar un corpus etiquetado representativo de bloqueos. PR #55 fusionado en `ebea11c`; CI `29970047220` y publicacion Usuario `29970343318` correctos. APK publica SHA-256 `5d38e574be73a368c1a4cccd9639da57d68058d27d595a578b40981a7eae78c0`, firma DEV historica; Admin permanece en 275. El usuario confirmo desde un telefono no enlazado que la carga va mucho mejor con la APK publica. La cache entre paginas por hash y NNAPI adaptativo siguen sin habilitarse sin invalidacion y benchmarks seguros.
...
- Ambas apps agregan en Ayuda un chat interactivo local, privado y limitado a Content Filter. Las preguntas sugeridas cambian con VPN, Accesibilidad, antidesinstalacion, sincronizacion y estado agregado de dispositivos; no usa una API externa, no envia preguntas y funciona offline en todos los Android compatibles con las apps.
- Recuperacion offline: Admin prepara por dispositivo un kit de cinco codigos de un solo uso. Solo salt/verificador y slots consumidos se sincronizan; los codigos legibles quedan cifrados localmente con Android Keystore. Usuario valida offline, conserva el bloqueo de 15 minutos tras cinco fallos y abre como maximo diez minutos para desinstalar. El consumo pendiente se concilia al recuperar conexion y los kits anteriores quedan invalidados por revision.
- Posible desinstalacion: el diagnostico exige administrador de dispositivo desactivado y mas de 30 minutos sin comunicacion; un cron DEV cada 15 minutos persiste una unica `ALERTA MAXIMA` por episodio con instrucciones para comprobar instalacion, reinstalar/reenlazar y restablecer protecciones. No afirma una desinstalacion sin evidencia y no depende del telefono posiblemente ausente para crear la alerta.
- Backend aplicado solo en Supabase DEV `syeycayasyufedwoprea`: migraciones `20260720153501_offline_recovery_kit.sql` y `20260720160558_possible_uninstall_alerts.sql`. La funcion de alerta solo puede ejecutarla `postgres/service_role`, el cron esta activo y los asesores DEV de seguridad/rendimiento no reportaron errores de nivel error. No se creo una alerta de prueba; el cron genero despues una alerta real de posible desinstalacion para un dispositivo efectivamente deshabilitado y sin comunicacion. Production no fue tocado.
...
- El contrato nuevo se conserva separado y retrocompatible: `list_device_announcements_v2`, `mark_device_announcements_read`, `dismiss_device_announcement` y `restore_device_announcement`. La tabla `device_announcement_receipts` no admite acceso directo desde Android, usa RLS y RPC autenticadas con el token del propio dispositivo. Sus FKs estan indexadas; sólo los recibos auxiliares acompañan en cascada un borrado definitivo del dispositivo ya autorizado en Superweb, mientras los avisos conservan FK restrictiva. No contiene Service Role ni secretos y la migracion no borra datos.
...
- App Admin expone el estado con el nombre claro `Proteccion contra desinstalacion`, elimina la opcion de otorgar mantenimiento general y conserva solamente `Permitir desinstalacion` por 30 minutos y la generacion del codigo de emergencia. Una autorizacion historica de mantenimiento todavia vigente se informa hasta vencer, pero ya no puede crearse desde la interfaz.
