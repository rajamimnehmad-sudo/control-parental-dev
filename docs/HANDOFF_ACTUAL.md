...
## DAG-V2-BROWSER-FOUNDATION-02 - candidato DEV local, revisión externa y física pendientes - 2026-07-25
...
- El Lab se ejecuta en `:dag2`, configura el directorio WebView `dag2` antes de crear WebView y corta la inicialización propia del proceso principal antes de VPN, WorkManager, sincronizaciones, receptores, monitores o servicios. DAG v1 sigue siendo el navegador predeterminado; su motor visual, modelos, thresholds, cachés y calibración no cambiaron. El único adaptador en su superficie es la opción interna gated de DEV.
- Cada documento usa `sessionId` y `navigationToken`; el análisis completo se inicia una sola vez y mantiene `fullPageAnalysisCount == 1`. Hash, `pushState`, `replaceState`, menús, filtros, modales, carruseles, botones, scroll y lazy loading no crean otra sesión. Una navegación nueva cancela la anterior y los callbacks o recursos con token obsoleto se descartan sin alterar la sesión vigente.
- La búsqueda reutiliza `dag-search` mediante un adaptador neutral: consulta, URL, dominio y resultados se filtran localmente; una consulta adulta se bloquea antes de Brave y una regla Admin común de permitir no reemplaza una prohibición adulta.
- HTML, CSS, JavaScript, JSON, XHR, `fetch`, fuentes, RSC, manifests, módulos y scripts de Service Worker devuelven `null` inmediatamente desde el router y nunca esperan trabajo visual. WebView y Service Workers usan el mismo router; fuera de una sesión v2 activa el cliente global de Service Worker devuelve `null`.
- En este lote no existe ruta visual aprobada: raster y SVG reciben un placeholder constante generado localmente, sin descargar, conservar, difuminar ni derivar píxeles originales. `data:`, `blob:`, backgrounds, canvas, video y audio permanecen neutralizados. `Sin caché DEV` aplica `LOAD_NO_CACHE`, `clearCache(true)` y el mismo modo al Service Worker.
- La evidencia de arquitectura, matriz Frávega A-F, métricas sanitizadas, comandos físicos pendientes y rollback está en `docs/dag/v2/DAG_V2_BROWSER_FOUNDATION_DECISIONS.md`. La matriz automatizada local quedó correcta; WebView debugging está ausente y los manifiestos fusionados confirman Lab sólo en DEV. El archivo DAG v1 sigue verificable.
