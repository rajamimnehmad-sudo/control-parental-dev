...
- Backend aplicado solo en Supabase DEV `syeycayasyufedwoprea`: migraciones `20260720153501_offline_recovery_kit.sql` y `20260720160558_possible_uninstall_alerts.sql`. La funcion de alerta solo puede ejecutarla `postgres/service_role`, el cron esta activo, no se genero ninguna alerta de prueba y los asesores DEV de seguridad/rendimiento no reportaron errores de nivel error. Production no fue tocado.
- Validacion automatizada previa al versionado: `./gradlew test ktlintCheck` correcto, 1.308 tareas. Las pruebas nuevas cubren asistente acotado/contextual, estado desconocido, inferencia de posible desinstalacion, uso unico/rotacion de codigos y recuperacion concurrente. `versionCode 266` y notas quedan preparados para ambas apps; falta compilar el artefacto final, instalarlo y recorrerlo fisicamente antes de publicar.
- Operacion cloud preparada: `Publicar APKs DEV` deja de ejecutarse automaticamente al hacer push y pasa a `workflow_dispatch` con confirmacion booleana. Usa `ubuntu-latest`, Gradle cacheado y `supabase/setup-cli@v3` con CLI 2.109.0; la Mac queda reservada para ADB. La guia movil esta en `docs/PUBLICAR_DEV_REMOTO.md`.
