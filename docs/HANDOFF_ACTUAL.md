...
- Publicacion: ninguna. `versionCode` permanece en 262, no se instalo en el telefono y no se subio ningun APK. La migracion `20260719224124_device_announcement_inbox_state.sql` esta preparada exclusivamente para Supabase DEV `syeycayasyufedwoprea`, pero aun no fue aplicada: la Supabase CLI quedo esperando autorizacion del llavero de macOS y se cancelo sin mutar el backend. No se toco Production.
