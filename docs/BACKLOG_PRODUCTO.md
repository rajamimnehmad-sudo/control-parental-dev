...
- Estado: `Pilotos NO-GO; pendiente cola humana minima de recortes reales`. El 2026-08-09 se midieron 28 originales y 84 variantes: R3.1 pasó de detectar 6/7 peligrosas originales a solo 2/7 variantes con máscara elíptica contenida, con 8 degradaciones `filter` a `allow` entre pares. Dos pilotos de ampliacion no corrigieron esas variantes. Un piloto posterior de consistencia explícita redujo las degradaciones peligrosas de 8 a 4, pero dejó 5/7 falsos permisos enmascarados, creó 2/21 falsos filtros y empeoró originales de 1/7 a 2/7 falsos permisos. Todos quedan `NO-GO`. La variante histórica llamada `circle128_q45` no simula un recorte circular `object-fit: cover`: conserva aspecto y produce elipses. El siguiente intento requiere recortes cuadrados centrados reales y una cola humana preseleccionada pequeña, porque el recorte puede eliminar el hecho que justificó la etiqueta original. No seguir ajustando pérdidas o pesos antes de obtener esas etiquetas. Evidencia: `docs/dag/v3/GLOSHIA_R4_THUMBNAIL_REPAIR_PILOT_2026-08-09.md`. Este ticket debe revisarse obligatoriamente cuando el usuario pida entrenar, reentrenar o reemplazar GloshIA Visual. No autoriza por si mismo cambiar R3, el modelo integrado, los umbrales ni la politica visual vigente.
