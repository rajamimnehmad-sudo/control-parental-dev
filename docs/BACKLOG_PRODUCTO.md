...
| DAG-CALIBRATION-CLOSED-LOOP-10 | Implementado candidato DEV 260; backend DEV aplicado y prueba fisica correcta | P0 | Hacer persistentes las decisiones calibradas, separar motivos positivos/negativos y agregar criterio local de mangas/corte sobre rodillas | L | Alto |
...
- App Usuario agrega dos señales tzniut locales y calibrables: manga por encima del codo y falda, short o pantalon por encima de la rodilla. Usa ML Kit Pose embebido solo cuando ya existe contexto femenino suficiente y combina landmarks de hombro/codo/muñeca o cadera/rodilla/tobillo con piel local. En SM-A235M la inferencia inicial midio 558 ms y las siguientes aproximadamente 70 ms; una apertura real de Zara cargo y siguio navegable. Es un detector beta con umbral conservador: los casos cercanos van a revision y la mejora continua depende de etiquetas humanas representativas, no se presenta como reentrenamiento de pesos.
