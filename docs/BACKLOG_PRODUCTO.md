...
| DAG-CALIBRATION-BIDIRECTIONAL-09 | Hotfix candidato DEV 258; pendiente prueba fisica | P1 | Modo temporal DEV que revela originales y permite X para falsos negativos o R para posibles falsos positivos, con trazabilidad separada | M | Alto |
...
- `DAG-CALIBRATION-BIDIRECTIONAL-09`: publicado en DEV 257; pendiente prueba fisica. Renombra la herramienta como `Calibración DEV`, recarga temporalmente la pagina con todos los raster originales ya evaluados y asigna `X` a los originalmente permitidos o `R` a los que DAG habria difuminado. X propone un falso negativo; R propone un falso positivo. Ambos llegan pendientes y separados a Super Admin, donde la etiqueta y el motivo humanos siguen siendo obligatorios antes de calibrar. El puente nativo se registra solo en DEV; desactivar el modo recarga la pagina y restaura el blur normal. La migracion esta aplicada solo en DEV, Edge v9 activa y Superweb desplegada.
