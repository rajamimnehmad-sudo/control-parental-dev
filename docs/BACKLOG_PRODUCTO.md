...
- Audio queda fuera de alcance y no se clasifica. DRM, vivo, PiP, casting,
  ...
  cerrados.
- Primer ticket: `DAG-VIDEO-01A0`, aprobado, implementado y validado fisicamente
...
- Estado: `Regresión de runtime corregida y validada físicamente en DAG 202`.
  La auditoría comprobó que Android había dejado de ejecutar `dag-36`: entre
  `0,40` y `0,95` permitía que cuadrantes débiles vetaran la señal completa y
  ya no usaba el contrato ordinario `0,30/0,45`. DAG 202 restaura completa
  `>=0,40`, cuadrantes ordinarios `0,30/0,45` y panoramas `0,50/0,70`, idénticos
  al Lab. R3.1, pesos y umbrales no cambiaron. R1, MobileNetV4 y TinyCLIP
  multiescala continúan `NO-GO`, sin checkpoints, ONNX o APK. Diagnostic 4
  comprobó además que Google no ofrece `srcset`, candidatos mayores ni
  `picture` para los raster relevantes. La matriz física de DAG 202 procesó
  Google Imágenes, Frávega, Mimo y Cheeky con colas p95 de 31-45 ms, cero
  crash/ANR, placeholders neutrales y menú de Mimo correcto después del scroll.
  Evidencia: `docs/dag/v3/GLOSHIA_R31_RUNTIME_GATE_2026-08-11.md`,
  `docs/dag/v3/GLOSHIA_R4_THUMBNAIL_REPAIR_PILOT_2026-08-09.md` y
...
- Problema 2, imagen completa frente a regiones: `corregido en código en DAG
  202`. Permitir señales completas de aproximadamente `0,7847` y `0,8419`
  porque cuadrantes débiles las vetaban no era la política canónica; fue una
  regresión de `4213d87`. Android vuelve a bloquear toda completa `>=0,40` y los
  tests anteriores que autorizaban `0,8` fueron reemplazados por el contrato
  correcto. Confirmado físicamente en el S22 con DAG 202.
...
- Orden de decisión: validar físicamente DAG 202 antes de reabrir entrenamiento.
  Cualquier modelo nuevo debe evaluarse con la política canónica ahora
  restaurada, sin mover umbrales ni usar el runtime divergente como baseline.
