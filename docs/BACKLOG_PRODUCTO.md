...
- Estado: `Pilotos NO-GO; cola humana mínima preparada`. El 2026-08-09 se midieron 28 originales y 84 variantes: R3.1 pasó de detectar 6/7 peligrosas originales a solo 2/7 variantes con máscara elíptica contenida, con 8 degradaciones `filter` a `allow` entre pares. Dos pilotos de ampliacion no corrigieron esas variantes. Un piloto posterior de consistencia explícita redujo las degradaciones peligrosas de 8 a 4, pero dejó 5/7 falsos permisos enmascarados, creó 2/21 falsos filtros y empeoró originales de 1/7 a 2/7 falsos permisos. Todos quedan `NO-GO`. La variante histórica llamada `circle128_q45` no simula un recorte circular `object-fit: cover`: conserva aspecto y produce elipses. Se generaron y puntuaron 642 recortes cuadrados centrados reales y se seleccionaron 24 para revisión (12/12 por etiqueta padre); el manifiesto no contiene `target` entrenable y marca `training_authorized: false`. La inspección confirmó que algunos recortes eliminan la persona/señal y otros la enfatizan, por lo que no se heredarán etiquetas. No seguir ajustando pérdidas o pesos antes de revisar esa cola. Evidencia: `docs/dag/v3/GLOSHIA_R4_THUMBNAIL_REPAIR_PILOT_2026-08-09.md`. Este ticket debe revisarse obligatoriamente cuando el usuario pida entrenar, reentrenar o reemplazar GloshIA Visual. No autoriza por si mismo cambiar R3, el modelo integrado, los umbrales ni la politica visual vigente.
