...
- Estado: `Corrección y Android CI validados; validación física SM-S908E pendiente`. Autorizado explícitamente el 2026-07-24, pero no declarado aprobado. Tipo: navegador, seguridad y arquitectura. Prioridad: P0. Esfuerzo: XL. Riesgo: alto.
...
- Aceptación: el comando exacto de Android CI, unitarios, aislamiento v1/v2, ktlint, Android Lint, Detekt y APK DEV correctos; el test del flag DEV fue movido de `src/test` a `src/testDev`, causa del fallo previo en flavors Beta. El run correctivo `30182570727` completó Android CI correctamente. El modo sin caché usa `LOAD_NO_CACHE` y limpieza explícita; WebView debugging está ausente; Beta y Production no exponen el Lab; los modelos, assets, thresholds, calibración y decisión visual DAG v1 no cambiaron, y su archivo verifica 28 archivos.
- Limitación: ADB detectó sólo el SM-A235M `R58T34V31AE`, que no se instaló ni modificó; el SM-S908E requerido no estaba conectado. Frávega, Mimo y Cheeky siguen pendientes en el S22 Ultra con diez aperturas normales, diez sin caché y veinte segundos por sitio. Los comandos y la matriz A-F están documentados; no se inventaron métricas físicas.
...
| DAG-V2-BROWSER-FOUNDATION-02 | Correccion y Android CI validados; validacion fisica SM-S908E pendiente | P0 | Base de navegador v2 separada, con todas las imagenes siempre neutras hasta existir el modelo propio | XL | Alto |
