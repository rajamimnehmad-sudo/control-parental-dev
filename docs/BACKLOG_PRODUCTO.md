...
- Estado: `Pilotos y rollback R1 NO-GO; revisión circular completa, candidato agrupado pendiente`. El 2026-08-09 se midieron 28 originales y 84 variantes: R3.1 pasó de detectar 6/7 peligrosas originales a solo 2/7 variantes con máscara elíptica contenida, con 8 degradaciones `filter` a `allow` entre pares. Dos pilotos de ampliacion no corrigieron esas variantes. Un piloto posterior de consistencia explícita redujo las degradaciones peligrosas de 8 a 4, pero dejó 5/7 falsos permisos enmascarados, creó 2/21 falsos filtros y empeoró originales de 1/7 a 2/7 falsos permisos. Todos quedan `NO-GO`. La variante histórica llamada `circle128_q45` no simula un recorte circular `object-fit: cover`: conserva aspecto y produce elipses. Se puntuaron 642 recortes cuadrados centrados reales y el propietario revisó 24: 10 allow, 11 filter y 3 doubt excluidas. La política privada confirmada aplica señales femeninas desde aproximadamente 5 años/edad incierta, no las aplica automáticamente a presentación masculina, no exige escote grande y cierra evidencia relevante insuficiente. En las 21 binarias R3.1 tiene 5/10 falsos filtros y 6/11 falsos permisos. R1 no recuperó más filtros en ese pool y elevó falsos filtros estimados a 7/10; en 112 vistas redujo falsos permisos estimados de 12 a 9, pero elevó falsos filtros de 5 a 26. El rollback R1 queda descartado y R1 se conserva solo como comparador. El pool privado está autorizado pero todavía sin split; falta un candidato A/B agrupado que repare miniaturas y positivos de cuerpos/escotes sin perder los negativos que R3.1 resuelve mejor. Evidencia: `docs/dag/v3/GLOSHIA_R4_THUMBNAIL_REPAIR_PILOT_2026-08-09.md`. Este ticket debe revisarse obligatoriamente cuando el usuario pida entrenar, reentrenar o reemplazar GloshIA Visual. No autoriza por si mismo cambiar R3, el modelo integrado, los umbrales ni la politica visual vigente.
