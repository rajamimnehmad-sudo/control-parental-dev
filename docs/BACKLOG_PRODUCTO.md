...
- Versiones DEV publicadas: App Usuario 307, App Admin 290 y DAG Browser 96.
...
- DAG 95 es la base rollback anterior de DAG 94: restaura el comportamiento
  ...
  DAG 96 y el codigo esta respaldado en GitHub.
...
- Estado: `Correctivo Usuario 304 instalado en SM-S908E; crash resuelto y
  mejora conversacional diferida hasta estabilizar el contexto de la app`.
  Aprobado
  explicitamente el 2026-07-20, ampliado
...
- Extension local: las fallas expresadas en el chat generan automaticamente un
  resumen técnico predeterminado para Superweb. Se permite categoría, versión,
  fabricante, modelo, Android y códigos de diagnóstico acotados; quedan
  excluidos el texto de la conversación, contraseñas, fotos, búsquedas y otros
...
- Evidencia SM-A235M: una pregunta libre con una contraseña sintética produjo
  ...
  En el SM-S908E, Usuario 304 conservó modelo, Accessibility y VPN; la llamada
  completa de LiteRT-LM 0.14 evitó el choque ABI del streaming y terminó sin
  caída. La salida probada fue rechazada de forma segura y volvió a la respuesta
  determinista. El diagnóstico final identificó `ungrounded`: el usuario decidió
  postergar la calibración avanzada hasta que las funciones de la app estén más
  terminadas. La barrera no se redujo y no se registró texto privado.
...
| LIC-REVOCATION-01A | Backend aplicado DEV y Android Usuario 311/Admin 293 validado; pendiente prueba fisica | P0 | Propagar cambios, comienzos y vencimientos de licencia con hora del servidor y revalidacion autorizada | M | Alto |
...
| DAG-HISTORY-UX-01 | Reimplementado en navegador V3 e instalado con DAG 31 | P2 | Redisenar historial DAG como lista minimalista | S | Bajo |
...
- Estado: `Backend aplicado DEV y Android Usuario 311/Admin 293 validado;
  pendiente prueba fisica`.
...
- Validacion: unitarios de `core-domain`, `core-data`, `core-network` y
  `core-sync`; Lint Vital y APK DEV locales de ambas apps; `git diff --check` y
  dry-run de Supabase correctos. La migracion
  `20260811004228_broadcast_license_invalidation.sql` quedo aplicada en DEV; el
  historial esta alineado y el dry-run final no tiene pendientes. La prueba de
  invalidacion notifico cuatro dispositivos sin cambiar la licencia; cron y
  trigger estan activos, los ciclos registrados terminaron correctamente y los
  asesores no agregaron hallazgos relacionados con este lote.
...
- Resultado posterior: DEV 277 fue revertido integramente en DEV 278 porque el usuario reporto una experiencia real lenta y torpe. DEV 278 restaura el codigo de comportamiento de DEV 276; las mediciones de DEV 277 quedan como evidencia historica y no como cierre vigente.
...
#### DAG-IMAGE-FILTER-1 - Prefiltro liviano de presencia humana

- Nombre de producto: `Filtro 1`. Estado: `Candidato descartado por regresion fisica; codigo retirado`. Tipo: rendimiento y seguridad visual. Prioridad: P1. Esfuerzo: M. Riesgo: alto.
- Objetivo: ejecutar primero un detector local muy liviano y mostrar rapidamente solo las imagenes confirmadas con alta seguridad como carentes de personas, rostros, cuerpos, piel relevante o ilustraciones humanas. Cualquier señal o duda deriva la imagen a todos los filtros de tzniut actuales.
- Seguridad: `no detectado` no equivale automaticamente a `seguro`. Recortes corporales, desnudez parcial, dibujos, reflejos, baja resolucion y piel sin rostro deben activar el analisis completo; errores, timeouts y formatos desconocidos fallan cerrados.
- Privacidad y recursos: todo permanece en el telefono, sin enviar fotos ni URLs a terceros. El prefiltro debe compartir decodificacion, mantener concurrencia acotada y demostrar que reduce tiempo y CPU sin aumentar memoria, temperatura ni falsos negativos.
- Aceptacion: comparar tiendas de electronica, ropa y paginas mixtas en el mismo Samsung; medir fotos liberadas por el prefiltro, derivadas al modelo completo, tiempo hasta pantalla completa, PSS, temperatura y bateria; validar un corpus de casos recortados y dificiles antes de habilitarlo por defecto.
- Candidato local 2026-07-23: el clasificador NSFW profesional sigue siendo obligatorio para todos los raster. Solo cuando ese modelo permite, el contexto no identifica una audiencia humana y MoveNet mas una comprobacion conservadora de piel coinciden en ausencia, se omite la capa ONNX corporal pesada. Personas, piel, contexto femenino/infantil, duda o error conservan el recorrido completo y fallo cerrado.
- Validacion: unitarios DEV, `ktlintCheck` y `assembleDevDebug` de App Usuario correctos; APK instalada in-place en SM-A235M sin incrementar version ni publicar. El recorrido y las metricas fisicas quedaron pendientes porque el dispositivo regreso a la pantalla de bloqueo.
- Resultado 2026-07-24: en SM-S908E el candidato agrego MoveNet/skin a casi todos los raster y solo evito la capa corporal en una fraccion minima. Fravega alcanzo `page_visible` en 11.502 ms; los logs mostraron mayoritariamente `filter1=full`. Se retiro exactamente ese prefiltro y se restauro el recorrido visual anterior, sin relajar filtros.
...
- Estado: `Retirado en DEV 275 por regresion de experiencia`; aprobado originalmente el 2026-07-22 y revertido con autorizacion explicita despues del reporte fisico. Tipo: rendimiento, clasificacion local y seguridad visual. Prioridad: P1. Esfuerzo: S. Riesgo: alto.
...
- Estado: `Implementado localmente; no publicado por pedido del usuario`. Tipo: UX/UI pura. Prioridad: P2. Esfuerzo: M. Riesgo: bajo.
...
#### GLOSHIA-VISUAL-TRAINING-01 - Consistencia en miniaturas y recortes regionales

- Estado: `Corpus dirigido y A/B de representación NO-GO; estudiar backbone compacto distinto`. El 2026-08-09 se midieron 28 originales y 84 variantes: R3.1 pasó de detectar 6/7 peligrosas originales a solo 2/7 variantes con máscara elíptica contenida, con 8 degradaciones `filter` a `allow` entre pares. Dos pilotos de ampliacion no corrigieron esas variantes. Un piloto posterior de consistencia explícita redujo las degradaciones peligrosas de 8 a 4, pero dejó 5/7 falsos permisos enmascarados, creó 2/21 falsos filtros y empeoró originales de 1/7 a 2/7 falsos permisos. Todos quedan `NO-GO`. La variante histórica llamada `circle128_q45` no simula un recorte circular `object-fit: cover`: conserva aspecto y produce elipses. Se puntuaron 642 recortes cuadrados centrados reales y el propietario revisó 24: 10 allow, 11 filter y 3 doubt excluidas. La política privada confirmada aplica señales femeninas desde aproximadamente 5 años/edad incierta, no las aplica automáticamente a presentación masculina, no exige escote grande y cierra evidencia relevante insuficiente. En las 21 binarias R3.1 tiene 5/10 falsos filtros y 6/11 falsos permisos. R1 no recuperó más filtros en ese pool y elevó falsos filtros estimados a 7/10; en 112 vistas redujo falsos permisos estimados de 12 a 9, pero elevó falsos filtros de 5 a 26. El rollback R1 queda descartado y R1 se conserva solo como comparador. El A/B de cinco folds comprobó que 10/21 recortes cambian legítimamente de etiqueta respecto del padre. La cabeza lineal mejoró falsos permisos OOF a 4/11, pero empeoró falsos filtros a 6/10 y fotos completas. El ajuste de última capa preservó fotos completas y mejoró variantes sintéticas, pero OOF empató exactamente a R3.1 en 6/11 y 5/10; ambos quedan `NO-GO`. Después se revisaron 120 grupos semánticos dirigidos: 59 allow y 61 filter, sin dudas ni duplicados. El propietario autorizó unirlos con los 42 claros previos: 162 grupos, 77 allow y 85 filter. R3.1 obtuvo 31 falsos permisos y 54 falsos filtros. El único A/B de representación quedó OOF 27/54 frente a gates 24/43 y un fold empeoró fotos completas; `NO-GO`, checkpoints eliminados. No variar más pesos sobre la misma última capa. Evidencia: `docs/dag/v3/GLOSHIA_R4_THUMBNAIL_REPAIR_PILOT_2026-08-09.md`. Este ticket debe revisarse obligatoriamente cuando el usuario pida entrenar, reentrenar o reemplazar GloshIA Visual. No autoriza por sí mismo cambiar R3, el modelo integrado, los umbrales ni la política visual vigente.
- Problema 1, miniaturas diminutas: en Google Imagenes se observaron sugerencias circulares de aproximadamente 976-1437 bytes que R3 permitio con puntajes cercanos a 0,025-0,20, mientras equivalentes grandes de 18-36 KiB alcanzaron aproximadamente 0,98-0,995 y fueron bloqueados. La clasificacion no debe depender de que el mismo contenido llegue grande, pequeño, comprimido, circular o levemente borroso.
- Segunda revisión 2026-08-09: el propietario aceptó la propuesta asistida y corrigió el caso 17 a `filter` por piernas de mujer. La unión privada quedó en 42 grupos claros (18 allow/24 filter) y 6 dudas excluidas. R3.1 obtuvo 16 falsos permisos y 11 falsos filtros. La cabeza lineal OOF mejoró a 10/13, pero sobrebloqueó y dañó validation; el ajuste de última capa empató exactamente 16/11. Ambos quedan `NO-GO` y sus checkpoints fueron retirados. No repetir variaciones de pesos sobre este pool. Próxima propuesta: corpus semántico dirigido de piernas femeninas, escotes, hombros, sujetos pequeños y negativos cercanos, preetiquetado de forma asistida y agrupado por persona/sesión/origen; requiere autorización antes de prepararlo o entrenarlo.
- Corpus dirigido y A/B: los 120 grupos nuevos terminaron en 59 allow y 61 filter, sin dudas ni duplicados. Autorizados para experimento privado, se unieron con 42 grupos previos. El candidato de última capa mejoró falsos permisos OOF de 31 a 27, dejó falsos filtros en 54 y dañó un fold fijo; no pasó los gates 24/43. Estado: `NO-GO`, sin checkpoints, ONNX, APK ni integración. Próximo paso: investigar un backbone compacto distinto antes de autorizar otro entrenamiento.
- Reentrenamiento estilo R1: por decisión explícita del propietario se repitió una sola vez la receta original desde TinyCLIP preentrenado, con cabeza logística nueva, ocho épocas acotadas, etiquetas humanas sin peso especial, sin R3.1 como maestro y sin variantes automáticas en train. OOF quedó en 9 falsos permisos y 56 falsos filtros sobre los 162 grupos, frente a 31/54 de R3.1 y 30/57 estimados para R1. Corrigió omisiones, pero agregó bloqueos inocentes y los cinco folds degradaron fotos completas a 2/7 falsos permisos; `NO-GO`. Se eliminaron checkpoints y código específico fallido, se conservaron informes/splits y no se tocaron R3.1, Android, APK, umbral, `frozen_test` ni `final_sealed`. No repetir esta receta; el próximo experimento requiere una representación compacta distinta y autorización separada.
- Cierre de representaciones compactas: con gate congelado de máximo 15 falsos permisos y 54 falsos filtros sobre los 162 grupos, y preservación estricta del control fijo, se rechazaron en fold 0 la interpolación R3.1/reentrenamiento, una cabeza residual MLP sobre R3.1, MobileNetV4 Conv Small y TinyCLIP 40M/32. La MLP preservó el control pero no mejoró el retenido; MobileNetV4 dejó 22-35 falsos filtros fijos y TinyCLIP 40M/32 dejó 18-28, frente al máximo 5. No se justificaron cinco folds, fine-tuning completo, ONNX ni prueba Android. Apple MobileCLIP se descartó por licencia de pesos no comercial. Se retiraron checkpoints y entrenadores rechazados; R3.1 sigue oficial. Próximo paso: corpus independiente con negativos cercanos y positivos equivalentes, seguido por entrenamiento/destilación GPU bajo los mismos gates; requiere ticket y autorización nuevos.
- Corpus independiente cerrado: se rescataron 625 decisiones históricas del propietario, se excluyeron 18 dudas, un duplicado entre lotes y una coincidencia exacta contra el train R3.1 real. El plan final tiene 508 imágenes nuevas de train (396 allow/112 filter) y 97 de holdout (76/21), agrupadas por serie y con SHA-256 real separado del campo histórico inconsistente. R3.1 obtuvo 2 falsos permisos y 8 falsos filtros en el holdout. Una cabeza anclada mejoró falsos filtros a 6 pero empeoró permisos a 4; tres ajustes de representación del TinyCLIP actual —últimas tres capas con dos balances y las diez capas con LR bajo— fallaron antes en originales con 2-3 falsos permisos, límite 1. `NO-GO`; checkpoints y código fallido retirados. El constructor auditado del corpus queda vigente. No repetir fine-tuning local del mismo estudiante; próximo ticket posible es destilación GPU hacia un único TinyCLIP del mismo costo Android, con proveedor, presupuesto, licencia y apagado automático autorizados por separado.
- Profesores congelados NO-GO: DINOv2-Small Apache-2.0 produjo originales 3/3, 112 vistas 9/7, holdout 4/7 y dirigido 28/22; SigLIP Base Apache-2.0 produjo 3/2, 13/6, 3/7 y 38/9. Ambos recuperan negativos pero permiten demasiados positivos, por lo que no corresponde destilar sus salidas actuales. Próximo paso propuesto: ajustar primero un profesor con el train humano, validarlo sin abrir exámenes sellados y recién entonces destilar hacia TinyCLIP. Opción recomendada: RunPod Secure Cloud RTX 4090, máximo USD 3/cuatro horas y apagado automático. Requiere autorización explícita para gasto, proveedor y carga privada; no está autorizada por este ticket.
- Problema 2, imagen completa frente a regiones: se observaron miniaturas con señal completa alta, aproximadamente 0,7847 y 0,8419, que terminaron permitidas porque sus cuatro recortes contextuales tuvieron señal baja. La corroboracion regional actual fue incorporada deliberadamente para reducir falsos positivos y tiene pruebas que permiten una imagen completa en 0,8 cuando todos los cuadrantes quedan en 0,1-0,2; no debe retirarse ni endurecerse sin evidencia comparativa.
- Dataset requerido: pares de la misma imagen en resolucion completa y miniatura; variantes comprimidas, borrosas, circulares y recortadas; imagen completa junto con sus regiones/cuadrantes; positivos dificiles y negativos visualmente parecidos como avatares, logos, productos y banners inocentes. Las particiones de entrenamiento y validacion deben evitar que variantes de una misma imagen se filtren entre conjuntos.
- Validacion obligatoria: medir por separado falsos negativos y falsos positivos por tamaño, compresion, recorte y forma; exigir consistencia de decision entre el original y sus variantes; comprobar que una señal relevante no desaparezca al regionalizar; y conservar una regresion fisica en Mimo, Cheeky, Fravega y Google Imagenes para proteger banners, productos y contenido inocente.
- Orden de decision: primero entrenar y evaluar el modelo nuevo con la politica vigente sin cambios. Solo si el modelo validado sigue fallando, experimentar en LAB con una regla de señal completa muy alta; no llevar ese cambio a la aplicacion ni modificar umbrales sin comparacion A/B y autorizacion expresa.
- Recordatorio operativo: ante el pedido `entrenar GloshIA`, abrir este ticket antes de preparar corpus, entrenamiento o integracion y recordar al usuario estos dos objetivos pendientes y sus puertas de regresion.
