...
const tab=tabs.find(t=>t.url.startsWith('https://www.mimo.com.ar'));
...
ws.onmessage=async e=>{const r=JSON.parse(e.data);if(r.id){pending.get(r.id)?.(r);pending.delete(r.id);return}
 if(r.method==='Debugger.scriptParsed'&&r.params.url.startsWith('https://www.mimo.com.ar')&&r.params.length>50000){
  const p=r.params, s=(await call('Debugger.getScriptSource',{scriptId:p.scriptId})).result?.scriptSource||'';
  const n=s.indexOf("reportBootstrapFailure(stage,reason);mutationDisconnect");if(n<0)return;
 ...
 if(r.method==='Debugger.paused'){
  ...
  const f=r.params.callFrames[0];if(f){const v=await call('Debugger.evaluateOnCallFrame',{callFrameId:f.callFrameId,expression:'JSON.stringify({stage:typeof stage!=="undefined"?stage:null,reason:typeof reason!=="undefined"?reason:null})',returnByValue:true});console.log(JSON.stringify(v.result||v.error))}
