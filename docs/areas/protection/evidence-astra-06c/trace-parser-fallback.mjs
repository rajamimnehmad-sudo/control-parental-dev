...
ws.onmessage=async e=>{const r=JSON.parse(e.data);if(r.id){pending.get(r.id)?.(r);pending.delete(r.id);return}
 ...
 if(r.method==='Debugger.paused'){
  ...
  const caller=r.params.callFrames[1];if(caller){const src=await call('Debugger.getScriptSource',{scriptId:caller.location.scriptId});console.log(JSON.stringify({callerSource:src.result?.scriptSource?.slice(0,1800)}))}await call('Debugger.resume');
 }
};
